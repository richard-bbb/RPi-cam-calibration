...
class VIDEO_ENCODER_STATE(Structure):
    _fields_ = [
        ...
        ("quantisationParameter", c_int),
        # Insert inline headers to stream (SPS, PPS)
        ("bInlineHeaders", c_int),
        ...
        ("inlineMotionVectors", c_int),
        # What intra refresh type to use. -1 to not set.
        ("intra_refresh_type", c_int),
...
arducam_set_video_callback.argtypes = [c_void_p, POINTER(
    VIDEO_ENCODER_STATE), OUTPUT_CALLBACK, c_void_p]
...
def check_status(status, func_name):
    if status != 0:
        raise RuntimeError(
            "{}: Unexpected result {}.".format(func_name, status))
...
class mipi_camera(object):
    ...
    def set_resolution(self, width, height):
        ...
        check_status(
            arducam_set_resolution(self.camera_instance,
                                   byref(_width), byref(_height)),
            sys._getframe().f_code.co_name
        )
    ...
    def start_preview(self, fullscreen=True, opacity=255, window=None):
        ...
        check_status(
            arducam_start_preview(self.camera_instance, byref(
                preview_params) if preview_params != None else None),
            sys._getframe().f_code.co_name
        )
    ...
    def set_raw_callback(self, func=None, userdata=None):
        ...
        cfunc = OUTPUT_CALLBACK(func) if func != None else cast(
            None, OUTPUT_CALLBACK)
        check_status(
            arducam_set_raw_callback(self.camera_instance, cfunc, byref(
                userdata) if userdata != None else None),
            sys._getframe().f_code.co_name
        )
    ...
    def software_auto_white_balance(self, enable=True):
        check_status(
            arducam_software_auto_white_balance(
                self.camera_instance, int(enable)),
            sys._getframe().f_code.co_name
        )
    ...
    def read_sensor_reg(self, address):
        ...
        check_status(
            arducam_read_sensor_reg(
                self.camera_instance, address, byref(_value)),
            sys._getframe().f_code.co_name
        )
        return _value.value
